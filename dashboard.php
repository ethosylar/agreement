		...
		<script>
			$(document).ready(function () {
				...
				$('#departmentFilter').on('change', function () {
					var departmentFilter = $(this).val();
					table.column(0).search(departmentFilter).draw();
				});
						...
						<?php
							...
							if ($result->num_rows > 0) {
								while ($row = $result->fetch_assoc()) {
									...
									$m = (int)$row['monthsLeft'];
									if      ($m <  0) $cls = 'over3months';
									elseif  ($m <= 3) $cls = 'lower3months';
									else              $cls = 'higher3months';
									echo '<td class="' . $cls . '">' 
									. htmlspecialchars($m) 
									. '</td>';
									// Created at
									...
									// Updated at (dash if NULL or empty)
									...
									echo '<td>
									<form action="view1.php" method="get" style="display:inline;">
